...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog w-75" role="document">
    <div class="modal-content mt-5">
      <img class="w-100 modalImage" :src="`https://seasonsge.com/api/upload/${image}`" alt="" />
    </div>
  </div>
</div>
    ...
    <div class="program-book d-flex">
        <div class="inner-box bg-white w-100 rounded-1 px-3">
            ...
            <div class="flight-trip carousel row justify-content-between px-5 mt-5" v-if="programDetails.includes_flight == '1'">
                <!--بداية ريسبونزف السلايدر-->
                  <!--الصور-->
  <div  :class="programmsImages.length == 0 ? 'd-none' : 'justify-content-center col-xl-6 col-lg-6 col-md-12 col-sm-12'" >
                    <carousel :autoplay="3000" :wrap-around="true" :items-to-show="1" class=" pb-5 w-100" style="border: none;margin-top: 150px;">
    <slide v-for="img in Array.from(new Set(programmsImages))" :key="img">
        <img v-if="img !=''" @click="uploadImageTomodal(img)" style="cursor: pointer;" data-toggle="modal" data-target="#exampleModal" class="w-100" :src="`https://seasonsge.com/api/upload/${img}`" alt="" />
    </slide>
    <template #addons>
      <navigation />
                        ...
                        <div class="more-info d-flex align-items-start mt-3 gap-4 flex-lg-row flex-column">
                            ...
                            <div class="flex-fill w-100">
                                ...
                                <div class="detail-label my-2">
                                    ...
                                    <span v-if="returnFlightNum" class="p-2 px-3 rounded-1 bg-secondary fw-bold bg-opacity-25 text-muted d-block">
                                        {{ returnFlightNum }} 
                                    </span>
                                </div>
                            </div>
                        ...
                        <label class="w-100 d-flex gap-2">
                            ...
                            <div class="w-100">
                                ...
                                <span v-if="validation.personsForm.$each.$response.$data[index].type.$error
                                    " class="error text-danger fst-italic d-block">
                                    {{
                                        validation.personsForm.$each.$response.$errors[index].type[0]
                                            .$validator === "required"
                                        ? "Field Required"
                                        : ""
                                    }}
                    ...
                    <div class="box d-flex gap-3 mt-3 flex-md-row flex-column">
                        ...
                        <label class="w-100">
                            ...
                            <span class="p-2 px-3 rounded-1 w-100 bg-secondary bg-opacity-25 d-block w-100">
                                <label class="position-relative mb-1">
                                {{ $props.searchInfo.includeFlight != 1 ? 'Land-Trip' : $t("programs.cardInfo.trip.flightLine") }}
                            </label>
                            </span>
                        </label>
                    </div>
...
const submission = async () => {
   /* let getImage;
    let postImage;
    let getData =  await axios.get('https://api.seasonsge.com/program-booking');
    if(getData.status == 200){
        getImage = getData.data;
       // console.log(getImage);
    }
    console.log('iam fetch');
   await axios.post('https://api.seasonsge.com/program-booking' , {
        ...
        if (localStorage.getItem("login")) {
            ...
            if (props.searchInfo.includeFlight == '1') {
                if (flightTrip.value.allowReturn != 1 && flightTrip.value.numTickets > bookingInfo.value.adults_count) {
                    ...
                    axios.post("https://api.seasonsge.com/ticket-out", tickets)
                        .then(data => {
                            console.log(data);
                        })
...
if(props.searchInfo.includeFlight == 1){
    await axios.get(`https://api.seasonsge.com/airline_id?id=${filterImages[0].flight_number}`)
.then((flightData)=>{
...
onMounted(async () => {
    ...
    await axios.get("https://api.seasonsge.com/cities-view").then((data) => {
    ...
    await axios
        .get(`https://api.seasonsge.com/pr-data?id=${props.programId}`)
        .then((data) => {
            // console.log(data);
